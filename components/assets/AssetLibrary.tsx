  ...
  return (
    <Card className="relative overflow-hidden group border-0 bg-transparent shadow-none hover:bg-muted/50 transition-colors rounded-lg">
        ...
        trigger={
          <div className="aspect-[3/4] w-full bg-muted rounded-md overflow-hidden relative cursor-pointer border border-border/50 hover:border-primary/50 transition-colors">
            {coverImage ? (
              ...
              <div className="flex h-full w-full items-center justify-center text-[10px] text-muted-foreground bg-muted/30">
                无图片
              </div>
            )}
            {count > 1 && (
                <div className="absolute bottom-1 right-1 bg-black/60 text-white text-[8px] px-1 py-0.5 rounded-sm backdrop-blur-sm">
                    +{count - 1}
                </div>
            )}
      ...
      <div className="pt-1.5 px-0.5">
        <h3 className="truncate text-[10px] font-medium leading-none mb-0.5">{asset.name}</h3>
        <p className="truncate text-[8px] text-muted-foreground">{asset.type === 'character' ? '角色' : '场景'}</p>
      </div>
...
export function AssetLibrary({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("flex h-full w-80 flex-col border-r bg-muted/10", className)}>
